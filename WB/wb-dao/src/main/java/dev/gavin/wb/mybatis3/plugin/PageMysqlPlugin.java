...
/**
 * <p>
 * mysql 分页插件，主要是拦截原内存分页方法，修改 sql 查询语句加入 limit(?,?) 实现数据库物理分页
...
public class PageMysqlPlugin implements Interceptor {

    private Logger logger = LoggerFactory.getLogger(PageMysqlPlugin.class);
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        logger.debug("rowBounds parameters limit: {}, offset: {}", rowBounds.getLimit(), rowBounds.getOffset());

        String sql = target.getBoundSql().getSql();
        StringBuffer buffer = new StringBuffer();

        logger.debug("sql append before: {}", sql);
        buffer.append(sql).append(" LIMIT ").append(rowBounds.getOffset()).append(",").append(rowBounds.getLimit());
        sql = buffer.toString();
